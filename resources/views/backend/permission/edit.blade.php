                    ...
                    <form method="POST" action="{{ route('backend.permission.update', $permission->id) }}">
                        ...
                        <div>
                            ...
                            <x-input id="name" class="block mt-1 w-full" type="text" name="name"
                                     :value="old('name', $permission->name)" placeholder="e.g. Access Post"
                                     required autofocus/>
                        </div>
                        <div class="mt-3">
                            <x-label for="identifier" :value="__('Identifier')"/>
                            <x-input id="identifier" class="block mt-1 w-full" type="text" name="identifier"
                                     :value="old('identifier', $permission->identifier)" placeholder="e.g. post_access" required autofocus/>
                        </div>
                        <div class="mt-3">
                            ...
                            <x-select id="module" class="block mt-1 w-full" name="module_id">
                                @forelse($modules as $module)
                                    <option
                                        value="{{ $module->id }}" {{ $module->id == old('module_id', $permission->module_id) ? 'selected' : '' }}>{{ $module->name }}</option>
                                @empty
