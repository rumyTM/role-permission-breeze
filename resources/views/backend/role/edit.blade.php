                    ...
                    <form method="POST" action="{{ route('backend.role.update', $role->id) }}">
                        ...
                        <div>
                            ...
                            <x-input id="name" class="block mt-1 w-full" type="text" name="name"
                                     :value="old('name', $role->name)" placeholder="e.g. Author"
                                     required autofocus/>
                        </div>
                        <div class="mt-3">
                            <x-label for="identifier" :value="__('Identifier')"/>
                            <x-input id="identifier" class="block mt-1 w-full" type="text" name="identifier"
                                     :value="old('identifier', $role->identifier)" placeholder="e.g. author" required autofocus/>
                        </div>
                            ...
                            @foreach($modules as $module)
                                @if($module->permissions->count())
                                    <div class="mt-2 sm:mt-0 sm:col-span-1 flex flex-col">
                                        <x-label for="name" :value="$module->name"/>
                                        @foreach($module->permissions as $permission)
                                            <label class="inline-flex items-center mt-1">
                                                <x-checkbox :value="$permission->id" :label="$permission->name"
                                                            name="permissions[]"
                                                            :checked="in_array($permission->id, old('permissions', $role->permissions->pluck('id')->toArray())) ? 'checked' : ''"/>
                                            </label>
                                        @endforeach
                                    </div>
                                @endif
